...
class CocoDetDataset(Dataset):
    ...
    def _anns_to_boxes_labels(self, anns: List[Dict]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for a in anns:
            ...
            cid = a["category_id"]
            labels_retina.append(self.catid_to_idx_for_retina[cid])
            labels_detr.append(self.catid_to_idx_for_detr[cid])
            areas.append(w * h)
        if not boxes:
    ...
    def __getitem__(self, idx: int):
        ...
        if self.augment and self.use_albu and self.albu_train is not None and boxes.shape[0] > 0:
            ...
            if self.model_family == "detr":
                t_labels = torch.as_tensor(transformed["class_labels"], dtype=torch.int64)
            else:
                t_labels = torch.as_tensor([l + 1 for l in transformed["class_labels"]], dtype=torch.int64)
            target = {
...
def show_image_with_boxes(img_tensor: torch.Tensor, target: Dict, catid_to_name: Dict[int, str], model_family: str):
    ...
    for box, lbl in zip(boxes, labels):
        ...
        if model_family == "retinanet":
            lbl_name = catid_to_name[lbl-1] if (lbl-1) in catid_to_name else str(lbl)
        else:
            lbl_name = catid_to_name[lbl] if lbl in catid_to_name else str(lbl)
        ax.text(x1, y1-2, lbl_name, fontsize=10, color="yellow", bbox=dict(facecolor="black", alpha=0.5))
    plt.axis("off")
    plt.show()
...
if __name__ == "__main__":
    ...
    ap.add_argument("--augment", action="store_true", help="Apply augmentation (train) or not (val)")
    args = ap.parse_args()
    ...
    for i in range(3):
        img, target = ds[i]
        show_image_with_boxes(img, target, ds.catid_to_name, args.model)
